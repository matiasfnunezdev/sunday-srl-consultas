...
interface GetUserViewModelResponse {
	getUser: (accessToken: string, uid: string) => Promise<void>;
...
export default function GetUserViewModel(): GetUserViewModelResponse {
	...
	const UseCase = new GetUser(new UserAPIImplementation());

	async function getUser(accessToken: string, uid: string): Promise<void> {
		try {
			setLoading(true);

			const response = await UseCase.invoke(accessToken, uid);
